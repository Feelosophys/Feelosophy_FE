...
interface AuthPageProps {
  onClose: () => void;
  onAuthSuccess: (user: { id: string; name: string; email: string; avatar: string; role: string }) => void;
}
...
export function AuthPage({ onClose, onAuthSuccess }: AuthPageProps) {
  ...
  const handleDemoLogin = async (accountType: 'expert' | 'patient') => {
    setIsLoading(true);
    setError('');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mock successful login - check if email is expert
      const isExpertEmail = loginForm.email.includes('expert') || loginForm.email.includes('doctor');
      const mockUser = {
        id: '1',
        name: loginForm.email.split('@')[0],
        email: loginForm.email,
        avatar: isExpertEmail 
          ? 'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=150&h=150&fit=crop'
          : 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop',
        role: isExpertEmail ? 'expert' : 'patient'
      };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mock successful registration - check if email is expert
      const isExpertEmail = registerForm.email.includes('expert') || registerForm.email.includes('doctor');
      const mockUser = {
        id: '2',
    ...
    <Card className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-sm border-blue-100">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center text-xs text-blue-600 bg-blue-50 p-3 rounded-lg">
          💡 <strong>Demo:</strong> Sử dụng demo accounts ở trên hoặc email có chứa expert hoặc doctor để truy cập với quyền chuyên gia
        </div>
      </CardContent>
    </Card>
  );
  ...
  const renderRegisterForm = () => (
    <Card className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-sm border-blue-100">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center text-xs text-blue-600 bg-blue-50 p-3 rounded-lg">
          💡 <strong>Demo:</strong> Sử dụng demo accounts ở trên hoặc email có chứa expert / doctor để đăng ký với quyền chuyên gia
        </div>
      </CardContent>
    </Card>
  );
  ...
  const renderVerifyCodeForm = () => (
    <Card className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-sm border-blue-100">
      <CardHeader className="text-center">
        ...
        <CardDescription>
          Nhập mã xác nhận đã được gửi đến {forgotPasswordForm.email}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 border-blue-200 focus:border-blue-400 text-center text-lg tracking-widest"
                maxLength={6}
                required
              />
            </div>

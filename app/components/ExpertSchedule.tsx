...
export function ExpertSchedule({ expert, onClose }: ExpertScheduleProps) {
  ...
  const groupedSlots = expert.availability.reduce(
    (acc, slot) => {
      if (!acc[slot.date]) {
        acc[slot.date] = []
      }
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>Chọn thời gian và thời lượng phù hợp để đặt lịch tư vấn với chuyên gia</DialogDescription>
      </DialogHeader>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start space-x-4">
              <ImageWithFallback src={expert.image} alt={expert.name} className="w-16 h-16 rounded-full object-cover" />
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-primary">{formatPrice(expert.price)}</div>
                ...
                <div key={option.value} className="relative">
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-gray-600 peer-checked:text-blue-700">{option.description}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-primary">{formatPrice(expert.price * option.value)}</div>
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-2xl font-bold text-primary">{formatPrice(calculateTotalPrice())}</div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{bookingSuccess ? "Đặt lịch thành công!" : "Xác nhận đặt lịch"}</DialogTitle>
            <DialogDescription>
              {bookingSuccess
          ...
          {!bookingSuccess ? (
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {selectedSlot &&
                        new Date(`${selectedSlot.date} ${selectedSlot.time}:00`).getTime() + duration * 60 * 60 * 1000 >
                          0 &&
                        new Date(
                          new Date(`${selectedSlot.date} ${selectedSlot.time}:00`).getTime() +
                            duration * 60 * 60 * 1000,
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-primary text-lg">{formatPrice(calculateTotalPrice())}</span>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="text-center space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-green-600 font-medium">Đặt lịch thành công!</p>
                ...
                <div className="bg-green-50 p-3 rounded-lg">
                  <div className="text-sm">
                    <div>
                      <strong>Tổng chi phí:</strong> {formatPrice(calculateTotalPrice())}
                    </div>
                    <div>
